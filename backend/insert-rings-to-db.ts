...
import { RingDB } from '../src/types/datatypes';

// Interface para o formato de dados extraídos de anéis
...
function convertToRingDBFormat(items: ScrapedItem[]): Omit<RingDB, 'id'>[] {
...
async function insertRingData(ringData: Omit<RingDB, 'id'>[]): Promise<boolean> {
  try {
    console.log(`Inserindo ${ringData.length} anéis no banco de dados...`);

    // Inserir dados na tabela 'ring'
    ...
    console.log(`✓ ${data?.length || 0} anéis inseridos com sucesso no banco de dados!`);
    return true;
  } catch (error) {
...
async function insertScrapedRingsToDatabase() {
  try {
    // Encontrar o arquivo mais recente de anéis extraídas
    const dataDir = path.join(process.cwd(), 'data');
    const files = await fs.readdir(dataDir);
    const ringFiles = files.filter(file => file.startsWith('scraped_rings_for_database'));
    
    if (ringFiles.length === 0) {
      console.error('Nenhum arquivo de anéis extraídas encontrado na pasta data/');
      return;
    }
    
    // Pegar o arquivo mais recente
    const latestFile = ringFiles.sort().pop();
    const filePath = path.join(dataDir, latestFile!);
    
    console.log(`Carregando dados de: ${filePath}`);
    
    // Carregar dados extraídos
    const scrapedRings: ScrapedItem[] = await loadScrapedData(filePath);
    
    console.log(`Dados carregados: ${scrapedRings.length} anéis`);
    
    // Converter para formato do banco de dados
    const ringDBFormat = convertToRingDBFormat(scrapedRings);
    
    // Inserir no banco de dados
    const success = await insertRingData(ringDBFormat);
...
// Executar a inserção
insertScrapedRingsToDatabase().catch(error => {
