...
import { LegsDB } from '../src/types/datatypes';

// Interface para o formato de dados extraídos de pernas
...
function convertToLegsDBFormat(items: ScrapedItem[]): Omit<LegsDB, 'id'>[] {
...
async function insertLegsData(legsData: Omit<LegsDB, 'id'>[]): Promise<boolean> {
  try {
    console.log(`Inserindo ${legsData.length} pernas no banco de dados...`);
    ...
    console.log(`✓ ${data?.length || 0} pernas inseridas com sucesso no banco de dados!`);
    return true;
  } catch (error) {
...
async function insertScrapedLegsToDatabase() {
  try {
    // Encontrar o arquivo mais recente de pernas extraídas
    const dataDir = path.join(process.cwd(), 'data');
    const files = await fs.readdir(dataDir);
    const legsFiles = files.filter(file => file.startsWith('scraped_legs_for_database'));
    
    if (legsFiles.length === 0) {
      console.error('Nenhum arquivo de pernas extraídas encontrado na pasta data/');
      return;
    }
    
    // Pegar o arquivo mais recente
    const latestFile = legsFiles.sort().pop();
    const filePath = path.join(dataDir, latestFile!);
    
    console.log(`Carregando dados de: ${filePath}`);
    
    // Carregar dados extraídos
    const scrapedLegs: ScrapedItem[] = await loadScrapedData(filePath);
    
    console.log(`Dados carregados: ${scrapedLegs.length} pernas`);
    
    // Converter para formato do banco de dados
    const legsDBFormat = convertToLegsDBFormat(scrapedLegs);
    
    // Inserir no banco de dados
    const success = await insertLegsData(legsDBFormat);
...
// Executar a inserção
insertScrapedLegsToDatabase().catch(error => {
