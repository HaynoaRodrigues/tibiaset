...
import { AmuletNecklaceDB } from '../src/types/datatypes';

// Interface para o formato de dados extraídos de amuletos e colares
...
function convertToAmuletDBFormat(items: ScrapedItem[]): Omit<AmuletNecklaceDB, 'id'>[] {
...
async function insertAmuletData(amuletData: Omit<AmuletNecklaceDB, 'id'>[]): Promise<boolean> {
  try {
    console.log(`Inserindo ${amuletData.length} amuletos no banco de dados...`);

    // Inserir dados na tabela 'amulet_necklace'
    ...
    console.log(`✓ ${data?.length || 0} amuletos inseridos com sucesso no banco de dados!`);
    return true;
  } catch (error) {
...
async function insertScrapedAmuletsToDatabase() {
  try {
    // Encontrar o arquivo mais recente de amuletos extraídos
    const dataDir = path.join(process.cwd(), 'data');
    const files = await fs.readdir(dataDir);
    const amuletFiles = files.filter(file => file.startsWith('scraped_amulets_for_database'));
    
    if (amuletFiles.length === 0) {
      console.error('Nenhum arquivo de amuletos extraídos encontrado na pasta data/');
      return;
    }
    
    // Pegar o arquivo mais recente
    const latestFile = amuletFiles.sort().pop();
    const filePath = path.join(dataDir, latestFile!);
    
    console.log(`Carregando dados de: ${filePath}`);
    
    // Carregar dados extraídos
    const scrapedAmulets: ScrapedItem[] = await loadScrapedData(filePath);
    
    console.log(`Dados carregados: ${scrapedAmulets.length} amuletos`);
    
    // Converter para formato do banco de dados
    const amuletDBFormat = convertToAmuletDBFormat(scrapedAmulets);
    
    // Inserir no banco de dados
    const success = await insertAmuletData(amuletDBFormat);
...
// Executar a inserção
insertScrapedAmuletsToDatabase().catch(error => {
