...
// Função para obter os IDs dos registros existentes na tabela 'rod'
async function getRodIds() {
  ...
  if (error) {
    console.error('Erro ao buscar IDs dos rod:', error);
    throw error;
  }
...
// Função para atualizar a tabela 'rod' com as URLs como imageUrl na coluna 'dados'
async function updateRodWithImages() {
  console.log('Carregando URLs de images.json...');
  const imageUrls = await loadImagesData();

  console.log('Buscando registros existentes na tabela rod...');
  const existingRod = await getRodIds();

  if (imageUrls.length > 0 && existingRod.length > 0) {
    const maxUpdates = Math.min(imageUrls.length, existingRod.length);
    console.log(`Atualizando ${maxUpdates} registros de rod com URLs de imagens...`);

    // Atualizar cada registro existente com a URL correspondente
    for (let i = 0; i < maxUpdates; i++) {
      const rodId = existingRod[i].id;
      const imageUrl = imageUrls[i];
      
      // Combinar o imageUrl com os dados existentes
      const existingDados = existingRod[i].dados || {};
      ...
      if (error) {
        console.error(`Erro ao atualizar rod com ID ${rodId}:`, error);
        throw error;
      } else {
        console.log(`✓ Rod com ID ${rodId} (${existingRod[i].name}) atualizado com imagem: ${imageUrl}`);
      }
    }

    console.log(`✓ ${maxUpdates} registros de rod atualizados com URLs de imagens.`);
  } else if (existingRod.length === 0) {
    console.log('Nenhum registro encontrado na tabela rod.');
...
async function main() {
  try {
    console.log('Iniciando processo de atualização da tabela rod com URLs de imagens...');
    await updateRodWithImages();
