...
// Função para obter os IDs dos registros existentes na tabela 'club'
async function getClubIds() {
  ...
  if (error) {
    console.error('Erro ao buscar IDs dos club:', error);
    throw error;
  }
...
// Função para atualizar a tabela 'club' com as URLs como imageUrl na coluna 'dados'
async function updateClubWithImages() {
  console.log('Carregando URLs de images.json...');
  const imageUrls = await loadImagesData();

  console.log('Buscando registros existentes na tabela club...');
  const existingClub = await getClubIds();

  if (imageUrls.length > 0 && existingClub.length > 0) {
    const maxUpdates = Math.min(imageUrls.length, existingClub.length);
    console.log(`Atualizando ${maxUpdates} registros de club com URLs de imagens...`);

    // Atualizar cada registro existente com a URL correspondente
    for (let i = 0; i < maxUpdates; i++) {
      const clubId = existingClub[i].id;
      const imageUrl = imageUrls[i];
      
      // Combinar o imageUrl com os dados existentes
      const existingDados = existingClub[i].dados || {};
      ...
      if (error) {
        console.error(`Erro ao atualizar club com ID ${clubId}:`, error);
        throw error;
      } else {
        console.log(`✓ Club com ID ${clubId} (${existingClub[i].name}) atualizado com imagem: ${imageUrl}`);
      }
    }

    console.log(`✓ ${maxUpdates} registros de club atualizados com URLs de imagens.`);
  } else if (existingClub.length === 0) {
    console.log('Nenhum registro encontrado na tabela club.');
...
async function main() {
  try {
    console.log('Iniciando processo de atualização da tabela club com URLs de imagens...');
    await updateClubWithImages();
