...
import { Item, EquippedItems, SlotType } from '../types';
...
const aggregateStats = (items: Item[]) => {
  ...
  items.forEach(item => {
    // 1. Parse Bonuses
    if (item.bonus) {
      // Regex for "Name +Amount" (e.g., "Magic Level +2", "Speed +20", "Dist +2")
      // Handles "Hit% +5" as well
      const match = item.bonus.match(/^(.+?)\s?\+?(\d+)$/);
      if (match) {
        const name = match[1].trim(); // "Magic Level"
              ...
              <div className="grid grid-cols-2 gap-4 animate-fadeIn mt-4">
                ...
                <div className="flex flex-col gap-2">
                   ...
                   {Object.entries(stats).map(([name, value]) => {
                      const colorStyle = getStatStyle(name);
                      return (
                ...
                <div key={item.id} className="group flex flex-col p-2 pl-3 rounded bg-slate-800/40 border border-slate-800 hover:bg-slate-700/50 hover:border-slate-600 transition-all duration-200">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <span className="text-sm font-medium text-slate-300 group-hover:text-white transition-colors truncate block">{item.name}</span>
                        {/* Bonuses in Log */}
